...
public class MailSendHandler {
    //日志记录器
    private Logger logger = LoggerFactory.getLogger(MailSendHandler.class);
        ...
        Thread thread = new Thread(() -> {
            while (true) {
                try {
                    ...
                    logger.info("从队列拿到邮件：{}", email.toString());
                    //第一种：不返回发送结果
                    executorService.execute(new MailSendWithoutResultTask(email));

                    //第二种：返回发送结果，并打印
//                    Future<String> future = executorService.submit(new MailSendWithResultTask(email));
//                    System.out.println(future.get());

                    Thread.sleep(200);//每隔200毫秒从队列取一次
